...
function exCards(list,isCoach){return list.map((e,i)=>`<div class="exc"><div style="width:52px;height:68px;flex-shrink:0;cursor:pointer" onclick="window.open('${e.url}','_blank')">${bodysvg(e.hl)}</div><div style="flex:1;min-width:0"><div style="font-weight:700;font-size:.85rem;margin-bottom:3px;cursor:pointer" onclick="window.open('${e.url}','_blank')">${e.n}</div><div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:3px"><span class="badge" style="background:${e.cat==='Force'?'var(--org)':e.cat==='Hypertrophie'?'var(--ac)':e.cat==='Machines'?'#06b6d4':e.cat==='CrossFit'?'var(--red)':e.cat==='Pilates'?'var(--pur)':'var(--grn)'};color:#000">${e.cat}</span><span class="badge ${e.type==='Poly'?'bbl':'bo'}">${e.type==='Poly'?'Polyarticulaire':'Isolation'}</span></div><div style="font-size:.72rem;color:var(--t3)"><strong style="color:var(--t2)">${e.m}</strong> · ${e.s}</div><div style="display:flex;justify-content:space-between;align-items:center;margin-top:4px"><span style="font-size:.65rem">Diff: ${ddots(e.diff)} · ${e.eq}</span>${isCoach?`<button class="b bs bsm" onclick="editExUrl(${EX.indexOf(e)})" style="font-size:.55rem">✏️ Vidéo</button>`:''}</div></div></div>`).join('');}
